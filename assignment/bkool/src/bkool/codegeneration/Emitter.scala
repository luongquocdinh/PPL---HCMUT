	...
	*/
    ...
    def emitALOAD(in:Type,frame:Frame) =  
      ...
      in match {
        ...
        case (ArrayType(_,_)|ClassType(_)) => jvm.emitAALOAD()
...
//  }
	...
	*/
	def emitREADVAR(name:String,inType:Type,index:Int,frame:Frame) = 
	{
      ...
      inType match {
        ...
        case StringType => jvm.emitALOAD(index)
        case (ArrayType(_,_)|ClassType(_)) => jvm.emitALOAD(index)
	...
	def emitREADVAR2(name:String,typ:Type,frame:Frame) = 
  {
	...
	*/
    ...
    inType match {
      ...
      case StringType => jvm.emitASTORE(index)
      case ArrayType(_,_) => jvm.emitASTORE(index)
      case _ => throw IllegalOperandException(name)
    }
  ...
  ////// CONST
  def emitWRITECONST(value: Expr, inType: Type, frame: Frame) =
  {
    frame.pop();
    inType match {
      case (IntType | BoolType) => emitPUSHCONST(value.isInstanceOf[IntLiteral].toString(), inType, frame)
      case FloatType => emitPUSHCONST(value.isInstanceOf[FloatLiteral].toString(), inType, frame)
      case StringType => jvm.emitLDC(value.isInstanceOf[StringLiteral].toString())
      case _ => throw IllegalOperandException(value.toString())
    }
  }
  
  ////// CONST
  def emitREADCONST(value: Expr, inType: Type, frame: Frame) =
  {
    frame.push();
    inType match {
      case (IntType | BoolType) => emitPUSHCONST(value.asInstanceOf[IntLiteral].value.toString(), inType, frame)
      case FloatType => emitPUSHCONST(value.asInstanceOf[FloatLiteral].value.toString(), inType, frame)
      case StringType => jvm.emitLDC(value.asInstanceOf[StringLiteral].value.toString())
      case _ => throw IllegalOperandException(value.toString())
    }
  }
    /* generate the second instruction for array cell access
  ...
  def emitWRITEVAR2(name:String,typ:Type,frame:Frame) = 
  {
	...
	*/
    ...
    case (Static,true) => ".field static final " + lexeme + " " + getJVMType(in) + " = " + value + "\n";
    ...
    def emitExpr(expr: Expr,typ:Type):String ={
    typ match{
      case IntType => expr.asInstanceOf[IntLiteral].value.toString()
      case FloatType=> if(expr.isInstanceOf[IntLiteral]) expr.asInstanceOf[IntLiteral].value.toFloat.toString()
                       else expr.asInstanceOf[FloatLiteral].value.toString()
      case StringType=> expr.asInstanceOf[StringLiteral].value
      case BoolType => expr.asInstanceOf[BooleanLiteral].value.toString() match{
        case "true" => "1"
        case "false"=> "0"
      }
    }
  ...
  * 
  */
  def emitINVOKESPECIAL() = jvm.emitINVOKESPECIAL()
  ...
  */
  ...
  def emitConcat(in:String, frame: Frame) = {
    frame.push()
    jvm.emitLDC( in);
  }
	/**
